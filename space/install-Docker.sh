...
install_package () {
	if ! is_command_exists $1; then
		...
		is_command_exists $COMMAND_NAME || { echo "Command $COMMAND_NAME not found"; echo xx 1; }
	fi
}
...
establish_conn() {
	...
	exec {FD}<> /dev/tcp/${1}/${2} && { exec {FD}>&-; echo "OK"; } || { echo "FAILURE"; echo xx 1; }
}
...
offline_check_docker_image() {
	[ ! -f "$1" ] && { echo "Error: File '$1' does not exist."; echo xx 1; }
...
dependency_installation() {
	...
	is_command_exists docker && { check_docker_version; systemctl start docker; } || { [ "${OFFLINE_INSTALLATION}" = "false" ] && install_docker || { echo "docker not installed"; echo xx 1; }; }

	if ! is_command_exists docker-compose; then
		[ "${OFFLINE_INSTALLATION}" = "false" ] && install_docker_compose || { echo "docker-compose not installed"; echo xx 1; }
	elif [ "$(docker-compose --version | grep -oP '(?<=v)\d+\.\d+'| sed 's/\.//')" -lt "21" ]; then
		[ "$OFFLINE_INSTALLATION" = "false" ]   && install_docker_compose || { echo "docker-compose version is outdated"; echo xx 1; }
	fi
}
